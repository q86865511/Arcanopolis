...
**M4.5-W1 到崗才算數完成**（629 tests 全綠、tsc 零錯誤、build 通過）：production 的在職判定
改為必須確認居民站在建築格上，走路動畫不再只是裝飾——通勤時間成為實際停工損失。
demo 世界 20 天糧食由 32392 降到 5472（日淨增 1600→244），平均到崗率 3.6/8 人。
下一步 M4.5-W2（建築進度條、存檔升 v5）。
...
- **M4.5 生產模型重做（插在 M5 之前，2026-08-23 使用者裁決走「到崗才算數＋建築進度條」）**：
  - ~~W1 到崗才算數~~ 已完成（見上）。
  - W2 建築進度條（模擬側）：Building 加 progress 欄位，累積滿才產出一批。
    存檔升 v5＋遷移函式＋舊檔載入測試；依專案規則為 Codex 第二審固定對象。
    待決：inputs 是開工時預扣還是完工時扣。
  - W3 進度條呈現＋數值重定：建築上方畫進度條；用快轉曲線重定 data/*.json。
  ...
  - [W1 登記] 玩家目前看不到單棟建築的實際效率（遠處的農場可能只跑 15% 卻毫無提示）。
    W3 的進度條天然就是這個指示器——條走得慢＝效率低，不必另外做 UI。
