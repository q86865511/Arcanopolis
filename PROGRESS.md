...
- **M4.5 生產模型重做（插在 M5 之前，2026-08-23 使用者裁決走「到崗才算數＋建築進度條」）**：
  - W1 到崗才算數：production 的在職判定改為「居民已抵達工作格」。
    目前 `production.ts` 只數 `citizen.job === building.id`，不看座標——居民還在路上走，
    建築就已滿產能運轉，走路動畫純屬裝飾。這是「產出與村民無連動」的根源。
  ...
  - **W1 必然引發的連鎖決定**：移動速度 0.1 格/tick、上半日 300 tick 朝工作地、下半日返家，
    所以有效工時 = max(0, 300 - 10×通勤距離)，最好情況也只有全日的 50%。
    通勤 24 格（現行上限）只剩 60 tick 有效工時＝現行產出的 1/10。
    W1 開工時必須在「提高移動速度」與「縮小可用半徑、讓佈局緊湊成為玩法」之間裁決。
