...
- **M4.5 生產模型重做（插在 M5 之前，2026-08-23 使用者裁決走「到崗才算數＋建築進度條」）**：
  ...
  - W4（可併入 W3）自動化平衡框架：腳本化玩家策略 → 目標帶斷言 → 常數網格搜尋。
...
- [登記 2026-08-23] 自動化平衡測試的三塊拼圖（見 M4.5-W4）：現有 `npm run simulate` 只是地基，
  缺玩家行為模型（否則曲線量的是靜止城市）、缺目標帶斷言、缺調參搜尋。
  誠實限制：自動調參只能滿足設定的目標帶，判斷不了好不好玩，目標帶仍是品味決定。
  **順序上必須等 M4.5 的產出模型定案後才建**，否則模型一換策略與目標帶全部作廢。
