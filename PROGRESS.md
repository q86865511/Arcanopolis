...
**M3.9 地形與世界完成**（486 tests 全綠、tsc 零錯誤）：遊戲現在跑在程序生成的島嶼上——
六型地形、地形綁定生產（伐木場需森林、採石場需石礦）、資源耗竭與森林再生、有界尋路、
分塊渲染與小地圖。**1000×1000 世界三根支柱全部實測可行**：地形隨用隨算（百萬格查 1 萬次 1.12ms）、
模擬（50 居民 100 tick 3ms）、渲染（初始化 1157ms、物件數與世界大小脫鉤）。
待使用者實玩驗收後進 M4（生產鏈與經濟，MVP 驗收里程碑）。
