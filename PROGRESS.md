...
~~W4 日夜視覺~~，M5 全數完成）→ M6 道路與城市規劃（完整設計已在
...
- [2026-09-03] **M5-W4 日夜視覺＋頂欄時鐘**（typecheck/805 tests/build 全綠，+10 tests；core 零改動）。
  ...
  夜色套在世界相機的 postFX ColorMatrix（`colorGrade.ts` 改回傳 handle，`setNight` 量化到 1/64 步才重建矩陣）：
  通道倍率 r 1−0.55s／g 1−0.45s／b 1−0.25s 偏藍、飽和再 −0.2s，最暗仍保留約 45% 亮度讓畫面可讀；
  uiCamera 不套所以 HUD 不變暗。頂欄新增「第N天 HH:MM 時段」（`hudLayout` 加時間保留寬）。
  同批加驗收用 URL 參數 `?new=1`（開新局不讀存檔）與 `?tick=N`（開局快轉），截圖腳本 `--query` 對應——
  白天／黃昏／夜晚三張截圖皆由此產出目視。分工：Codex 實作全部八檔，主對話重驗與截圖。
  未做：夜間窗戶亮燈（需素材，擇機）；細作息依裁決併入 M7。
- [2026-09-03] **M5-W3 建築選單改版：時代階段頁籤＋人口解鎖＋懸停資訊卡**（typecheck/795 tests/build 全綠，+48 tests）。
  `data/buildings.json` 每棟加 `unlockAtPopulation`，新表 `data/eras.json` 定三階段（村莊 0／小鎮 12／城市 24）；
  分組規則在 `src/data/eras.ts` 純函數（core 與 render 共用），core 的 placeBuilding 在成本檢查後加解鎖閘門
  （人口＝citizens.length，只在放置時檢查，之後掉回門檻以下不影響已建）。
  底欄改為頁籤列＋當前階段格子：未達門檻的頁籤顯示「小鎮 · 人口12」壓暗但可切過去預覽，
  未解鎖建築縮圖壓暗、點選出「需人口 N」提示；數字鍵對當前頁籤、`[` `]` 改為切階段。
  懸停格子浮出資訊卡（`BuildingInfoPanel` ＋ `buildingInfoText`/`buildingInfoLayout` 純函數）：成本（買不起標紅）、
  原料→產出（以一批換算）、工人／住房、地形限制、解鎖狀態。卡片錨在底欄頂邊避免壓住頁籤。
  `scripts/screenshot.mjs` 加 `--hover x,y` 與 `--click x,y`，懸停與切頁籤的視覺都有截圖證據。
  分工：資料契約與整合由主對話落地；core 閘門與資訊卡三檔由 Codex 實作；頁籤與接線由 Claude architect 實作。
  門檻取自 30 天平衡曲線（人口 6 起每日 +1：第 7 天到小鎮、第 19 天到城市），加閘門後曲線與加前完全一致
  （擴建策略本就在門檻後才蓋後段鏈）。登記：每階段建築數上限 10（數字鍵數），超過需階段內分頁，測試已鎖。
- [2026-08-27] **M5-W2 資源列與診斷回饋**（typecheck/747 tests/build 全綠）。
  資源列從「單一 monospace 文字」改為逐項元件：**24×24 圖示＋數值＋每日增減趨勢**
  （▲綠/▼紅/持平），人口項從「6/12」改為指出卡在哪一條——
  `diagnosePopulation` 逐條照抄 `population.ts` 的判別順序（餓死中／缺糧／住房已滿／成長中），
  **判斷邏輯在 render 純函數，core 零改動**。趨勢資料是 render 端每日界快照
  （`resourceDiagnostics.ts`，不寫回 GameState、不進存檔；存檔載入後顯示無趨勢）。
  排布抽成 `resourceRowLayout.ts` 純函數（降級序列：縮字級→隱藏趨勢欄→圖示恆保留）。
  素材：11 個資源圖示一次生成通過，art-bible 新增「UI 圖示前綴」段（正面平視、極簡剪影），
  新增 `scripts\process-icon-sprites.mjs`（最長邊縮 24 後置中補正方形）。
  **2026-08-31 使用者裁決：拿掉資源名標籤**，資源身分由圖示表達（更貼近 AoE2 的純圖示＋數字），
  數值欄估寬 7→5 字元。**殘留限制**：800px 窄視窗下 11 個 24px 圖示的固定寬度本身已超出預算，
  人口文字仍與尾端 1 項相碰（1280 以上無此問題）；圖示不隨窄視窗縮小是刻意設計，
  若要徹底消除需隱藏存量 0 的資源或換行，暫不做。
