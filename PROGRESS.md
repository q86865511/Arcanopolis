...
**M4-W2 經濟完成**（567 tests 全綠、tsc 零錯誤、build 通過）：在 W1 生產鏈之上補齊金幣循環——
就業居民每日繳稅（無業不繳），市場建築解鎖資源↔金幣交易，買價含 50% 價差杜絕套利。
W3 的建築素材批**已提前完成**：六棟加工鏈建築素材上線，畫面上不再有隱形建築。
建築選單改為分頁（`[` `]` 換頁），資料表再長都不會有選不到的建築。
下一步 M4-W3（建築選擇 UI、通勤半徑硬上限、平衡調校）＝ MVP 驗收停斷點。
...
- M4-W3 建築選擇 UI（含市場交易操作介面）、通勤半徑硬上限、遊戲速度檔位、拆除警告、
  平衡調校（MVP 驗收停斷點）；M5 塔防；M6 冒險者公會；M7 魔法；M8 種族；M9 上架。
- [登記 M4-W2] 市場只有核心與測試，**玩家還沒有操作介面**（無法選資源、無法下買賣單）。
  W3 的建築選擇 UI 必須一併把交易操作做出來，否則市場蓋了也用不到。
- [登記 M4-W2] 稅率 2 金幣/就業居民/日與市場價格表（wood 2 … tools 30）是首版估值，
  尚未用快轉曲線調校過；W3 平衡調校時以 `--full-sim` 曲線重新定值。
